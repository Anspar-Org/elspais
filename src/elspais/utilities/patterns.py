...
class PatternValidator:
    ...
    def _build_regex(self, include_assertion: bool = False) -> re.Pattern:
        ...
        pattern = pattern.replace(
            "{prefix}", f"(?P<prefix>{re.escape(self.config.prefix)})"
        )
    ...
    def parse(
        self, id_string: str, allow_assertion: bool = False
    ...
    def format_assertion_label(self, index: int) -> str:
        ...
        if style == "uppercase":
            if index < 0 or index >= 26:
                raise ValueError(
                    f"Index {index} out of range for uppercase labels (0-25)"
                )
            ...
            if index < 10:
                ...
                raise ValueError(
                    f"Index {index} out of range for alphanumeric labels (0-35)"
                )
        elif style == "numeric_1based":
            if zero_pad:
