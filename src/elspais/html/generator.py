...
# `neutral` (the forced N/A override, REQ-d00292-H) sits at the same low
...
_SEVERITY_TO_BUCKET: dict[str, str] = {
    ...
    "warning": "partial",
    "info": "full",
    # `neutral` (N/A override, REQ-d00292-H) is non-dragging exactly like `info`:
...
# Implements: REQ-d00069-L, REQ-d00258-A, REQ-d00258-J, REQ-d00291-H, REQ-d00292-G+H
def compute_coverage_tiers(node: GraphNode, config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    # Per-level UAT expectation (REQ-d00291-H). By default a UAT `missing` tier is
    ...
    for dim_key, dim, sev_cfg, prefix in dim_map:
        ...
        # see REQ-d00258-S and REQ-d00292-G) regardless of the dimension's
        ...
        # "fully-covered-including-indirect" state (REQ-d00292-A). This is a color
        ...
        # COLOR is decoupled from severity (REQ-d00292-F): it resolves from the
    ...
    # (red) UAT badge; a non-expecting one shows none (REQ-d00291-A, REQ-d00292-G).
    result["expects_validation"] = expects_validation

    return result


# The semantic per-assertion coverage "standings" (REQ-d00292-B). These are the
...
# (REQ-d00293-A). This keeps the standing->color association configurable and
# out of the badge logic.
COVERAGE_STANDINGS = ("full", "partial", "failing", "missing")


# Implements: REQ-d00292-F, REQ-d00293-A
def _standing_color(standing: str) -> str:
    """Resolve a coverage standing to its theme-catalog color_key (REQ-d00292-F).
...
# Implements: REQ-p00017-G, REQ-d00292-B+C
def compute_assertion_coverage_states(
    ...
    never disagree (REQ-d00258-T, REQ-d00292-C): if every assertion is ``"full"`` the
    ...
    def _passing_standing(passing: CoverageDimension, label: str) -> str:
        ...
        set the requirement-wide ``has_failures`` flag (REQ-d00292-D). The
...
# Implements: REQ-d00069-L, REQ-d00258-A, REQ-d00258-J, REQ-d00292-E
def compute_assertion_coverage_measures(node: GraphNode) -> dict[str, dict[str, str]]:
    ...
    produced it (REQ-d00292-E). It replaces the retired ``~`` caveat, which
