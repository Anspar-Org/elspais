...
def get_spec_directories(config: Dict[str, Any], base_path: Optional[Path] = None) -> List[Path]:
    """Get spec directories from configuration.
    ...
    Returns:
        List of absolute Path objects for spec directories
    """
    if base_path is None:
        base_path = Path.cwd()

    spec_dirs = config.get("spec", {}).get("directories", ["spec"])
    result = []
    for d in spec_dirs:
        path = Path(d)
...
def load_requirements_from_repo(repo_path: Path, config: Dict[str, Any]) -> Dict[str, Requirement]:
    ...
    if repo_config_path.exists():
        repo_config = load_config(repo_config_path).get_raw()
    else:
