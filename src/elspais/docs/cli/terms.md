...
While not a terms check, `code.no_traceability` (configured in `[rules.format]`) is closely related to the terms workflow. It reports code files holding a citation that reaches no requirement. Files that cite nothing at all are covered separately by the `code.uncited_file` check, and test files by `tests.uncited_file`. This helps identify source files that have not been linked to any requirement.
