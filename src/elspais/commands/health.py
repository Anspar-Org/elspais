...
def check_config_syntax(config_path: Path | None, start_path: Path) -> HealthCheck:
    ...
    actual_path = config_path if config_path and config_path.exists() else find_config_file(start_path)
...
def check_code_coverage(graph: TraceGraph) -> HealthCheck:
    ...
    return HealthCheck(
        ...
        message=f"{len(covered_reqs)}/{req_count} requirements have code references ({coverage_pct:.1f}%)",
        category="code",
        severity="info",
        details={
...
def check_test_results(graph: TraceGraph) -> HealthCheck:
    ...
    if failed > 0:
        return HealthCheck(
            name="tests.results",
            passed=False,
            message=f"Test failures: {passed} passed, {failed} failed, {skipped} skipped ({pass_rate:.1f}% pass rate)",
            category="tests",
            severity="warning",
            details={"passed": passed, "failed": failed, "skipped": skipped, "pass_rate": round(pass_rate, 1)},
        )
...
def check_test_coverage(graph: TraceGraph) -> HealthCheck:
    ...
    return HealthCheck(
        name="tests.coverage",
        passed=True,  # Informational only
        message=f"{len(covered_reqs)}/{req_count} requirements have test references ({coverage_pct:.1f}%)",
        category="tests",
        severity="info",
        details={
