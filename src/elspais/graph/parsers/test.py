...
class TestParser:
    ...
    # Captures: REQ_d00060, REQ_d00060_A, REQ_d00060_A_B (multi-assertion)
    ...
    TESTS_BLOCK_HEADER = re.compile(r"(?:#|//|--)\s*TESTS?\s+REQUIREMENTS?:?\s*$", re.IGNORECASE)

    # Multiline block item: --   REQ-xxx: Description  or  --   REQ-xxx
    BLOCK_REQ_PATTERN = re.compile(r"^\s*(?:#|//|--)\s+(?P<ref>REQ-[A-Za-z0-9-]+)")
            ...
            if self.TESTS_BLOCK_HEADER.search(text):
                ...
                while i < len(lines):
                    ...
                    if ref_match:
                        ...
                        next_text.strip().startswith(("#", "//", "--"))
                        and not next_text.strip().rstrip("#/-").strip()
                    ):
