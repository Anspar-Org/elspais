...
#### `summary` and `trace` publish the four coverage measures instead of a caveat marker

- **BREAKING: the `~` caveat marker is retired from `elspais summary` and `elspais trace`; both now headline the per-*Assertion* total and publish the measures behind it (REQ-d00258-A, REQ-d00258-J)** — coverage is measured on four independent axes crossing what a citation named (direct/indirect) with where the evidence sits (immediate/rolled-up), and a requirement's headline is the greatest of an *Assertion*'s four measures rather than either legacy blended footing. `~` used to flag a count that was not fully backed by direct evidence, standing in for a measure the cell did not otherwise show; a surface no longer annotates a figure with a caveat for a measure it withholds — it shows the measure instead. `summary`'s text and markdown renderers print each dimension's total followed by its four measures, naming each ("cited by name here" for immediate-direct, "whole-requirement" for immediate-indirect, "conducted direct"/"conducted indirect" for the two rolled-up measures); its CSV and JSON forms carry the same four measures as columns/fields beside each dimension. These words now canonically denote the four REQ-d00069-L measures on every surface: the `health` coverage check reads two LEGACY BLENDED footings that predate this split (`check_dimension_coverage` was not migrated by this change) and its labels for them were reworded to say so plainly ("legacy direct footing", "legacy indirect footing") rather than reuse "direct"/"indirect"/"conducted" for a different quantity. `trace` headlines the same total in every format, and `--format csv` gains four measure columns per dimension (`Tested Immediate Direct`, `Tested Immediate Indirect`, `Tested Rolled Direct`, `Tested Rolled Indirect`, and likewise for Implemented/Passing/UAT Covered/UAT Passed) immediately after that dimension's own breakdown columns. The REQ-d00258-O Tested breakdown (`[N passed, N failed, N awaiting a result]` / `[3P 0F 2A]`) and the `*`/`!`/`(baseline)` markers are unchanged — they answer different questions and were never what `~` stood in for. **Migration**: a parser reading `~` off `summary`/`trace` output should read the new measure columns/fields instead; nothing in the underlying coverage figures changed, only what is shown and how.
- **`tests.uncredited_evidence`'s chained-dimension membership now follows the project's configured footing (REQ-d00274-B)** — it read the generous footing unconditionally regardless of `[rules.coverage] allow_indirect`, so a project that configured the strict footing had its own chained figures leave an *Assertion* out of a dimension's denominator while this check still counted it in, silently disagreeing with the project's own answers. It now reads whichever footing `allow_indirect` selects, matching what the chained tier figures (health, the viewer badges) actually count.
