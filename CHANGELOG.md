...
- **`mutate_add_remainder` raised `AttributeError` and could never succeed (CUR-1829)** — `_mutate_add_remainder` called `graph.add_remainder(...)`, which existed on `TraceGraphBuilder` but was never delegated on `FederatedGraph`. Every call failed. The delegation is added.
