...
- **`match = "precise"` resolves results per test, not just per file (CUR-1533)** — `precise` matching for test_id-less reporters (`flutter-machine`) now correlates each result to the specific `test()` by `(source_file, line)`, with file-granular fallback when the line doesn't resolve (shared-helper or generated tests). This required teaching elspais to see Dart test-case boundaries: a new `dart_prescan` (regex + string/comment-aware brace matching, no Dart parser) routes `.dart` test files so each `test()`/`testWidgets()` becomes its own TEST node anchored on the call-site line, and `// Verifies:` comments attribute to individual tests. The builder resolves each precise result by `(source_file, line)` — preferring the single matching TEST, falling back to every TEST in the file — and stamps `precise_scope` (`"test"`/`"file"`). **Crediting follows resolution:** a line-resolved result credits per-test inline (its pass credits only its assertions; its fail flags only its own), while file-scope results keep the prior all-pass/any-fail file-level crediting via the annotator's `precise_index` (no double-counting — the two are complementary). This also wires the real RESULT→TEST `YIELDS` edges the viewer's per-assertion Verified panel reads (`_get_assertion_test_map` → `_serialize_test_info`), which were previously absent for `flutter-machine`. `flutter-machine` results now carry their source `line`; `test_id`-linked reporters (junit/pytest) are unchanged. `REQ-d00254-G` amended accordingly.
