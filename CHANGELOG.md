...
- **A save discarded the changes it declined to write (REQ-d00132-E, REQ-d00253-B, REQ-d00253-D, REQ-o00062-O)** -- a save writes only the repository the tool is serving unless `federation.write_associates` says otherwise, and it clears the pending changes once it has succeeded. A change queued against a file an associate owns met both of those at once: the file was held back, the pending record was cleared anyway -- and clearing it in a federation reaches into every member's own record -- so the save reported success, wrote the serving repository's file, left the associate's untouched, and destroyed the edit made to it with nothing in the report saying so. A held-back file that carries queued work is now reported to the caller, naming the file and why it was not written, and a save that left somebody's change unwritten is not a success, which is what keeps the pending changes: they are cleared only after a save that performed them. The work is still there afterwards, to be written by a tool serving the repository that owns it or by a save made with associate writes enabled. An unsuccessful save also says which kind of unsuccessful it was, because a caller can act on one and not the other: a save whose only complaint is a file it declined to write carries `write_scope_declined` and an error naming those files in full, and the viewer answers it with 403 rather than the 500 a failed write gets, so a policy answer a further save will repeat is told apart from a write that may succeed next time. A save that also failed to write something is an unnamed failure, because that is the part a caller can do least about. Declining one member's file is no reason to abandon another's, so a save can write the served repository's file, report it, and still be a decline. The MCP save tool and the viewer's route read the one save result, so both report the same code and the route hands the save's own body back rather than re-wording it; the viewer's page passes a refusal that names itself to the caller that raised it instead of collapsing it into a toast. A file that is merely parse-dirty -- formatting the tool would tidy, which nobody asked for -- is still held back in silence, so `elspais fix` in a federation is unaffected. Keeping the pending record is all or nothing: where the serving repository's own file was written, its entry is kept with the rest and a later save re-renders that file harmlessly, the direction the duplicate-identifier backstop beside it already takes, because a file written twice costs nothing and a lost edit cannot be recovered. The viewer's mutation routes now refuse a target an associate owns before they judge the version token, in the order the MCP tools check in and returning the tools' own refusal verbatim under HTTP 403: a write the agent surface refuses and the viewer accepts is a mutation reachable on one surface alone, and accepting it for an associate-owned node only queued an edit the save would then decline to write.
