...
- **A pull request is opened from the viewer (REQ-d00297)** -- `POST /api/git/pr` proposes the branch the session has pushed, and the page offers it as a small dialog the moment a push succeeds, with the title prefilled from the checkpoint suggestion. The pull request is attributed to whoever's credential opened it: the one the request carried under the proof of trust the identity headers use, else `GH_TOKEN` or `GITHUB_TOKEN` in the server's environment, else the credential helper; with none of those the request is refused naming what to supply, rather than answered by whatever identity the serving machine happens to hold. A branch the remote has not seen, a detached HEAD and a remote that is not GitHub's are each refused naming the condition, and where the remote already holds an open pull request proposing the same branch, that pull request is reported as the outcome.

  A credential a request carries now also serves the push and the fetch that request asks for. It is carried on that one command and reaches neither the repository's configuration, the environment the server was started from, nor any file. Where a pull cannot fast-forward, the viewer says so and tells the user to push the branch and finish the work in a local checkout -- elspais does not rebase, merge or resolve conflicts on their behalf.
