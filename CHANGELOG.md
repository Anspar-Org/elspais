...
- **A configuration change is reachable from the agent surface and the viewer's, and refused identically on both (REQ-o00062-O)** -- `mutate_add_declaration`, `mutate_update_declaration`, `mutate_rename_declaration` and `mutate_delete_declaration` on the MCP interface, and `POST /api/mutate/add-declaration`, `/api/mutate/update-declaration`, `/api/mutate/rename-declaration` and `/api/mutate/delete-declaration` on the viewer's, reach the same shared functions, so a person and an agent changing one configuration have the same capabilities and the same safety. Each takes a version token and refuses a stale one through the guard both surfaces already share: an addition names the document's token, because the declaration it adds does not exist yet to hold one, and the others name the declaration's own. A refusal carries the current version and the current state, so a caller reconciles what it meant against what is there rather than overwriting a change it never saw; over HTTP that is the tool's body unchanged, with a 409. Every result names the declaration the change left behind -- after a rename, not the identifier the caller supplied -- and reports the token to name next, a removal reporting none because nothing is left to hold one; where another document declares the same name, the result says so and says the effective configuration is unchanged. Writes on either surface serialize on the one lock the writers share, and the routes are classified as version-guarded by the parity suite rather than exempted from it.

- **A configuration document is changeable only in the repository the tool is serving (REQ-d00299-F)** -- a tool serving one repository writing into another's checkout is surprising wherever it happens, and where those checkouts were made by a host for a session, an edit to one would be discarded without anyone noticing. A change aimed at an associated member's configuration is refused, naming the document's own repository and the one the tool is serving, and saying to change it from a tool serving that repository instead. Ownership is asked of the document rather than of its identifier, through the one resolution every write-scope guard already uses: a repository-relative path repeats across members, so the question asked by identifier can be answered with a member that merely holds the same path. A refusal changes nothing, on disk or in memory, and joins no mutation log. Reading is unrestricted, and a federation held a configuration document for every member already -- what is new is that a write cannot cross the boundary a read has always respected, scopes having been read per project all along. A change asked for through either the MCP or the HTTP surface meets this refusal, both reaching the document by one path.

- **A change another document declares the same name under is reported as changing nothing a reader can see (REQ-d00299-G)** -- the only way a repository holds two configuration documents is the machine-local overlay layered over the committed file, and a name declared in both resolves to the overlay's declaration. A change to the committed document therefore moves nothing any consumer reads, and a change reported as applied is then indistinguishable from one that worked. The mutation carries the document the name actually resolves to, and carries nothing where nothing shadows the change, so a reader testing for the disclosure never has to read an absence as a value. Which document wins is answered by the same lookup that resolves a declaration for a reader, so the disclosure cannot drift from the precedence it describes. What is disclosed is where the name resolves and never what that document declares: REQ-d00290-B admits one coarse fact about a machine-local override and bars reporting which values an overlay contributed, and the fact stated here is about the write -- that it landed where nothing reads -- rather than about the overlay's contents. A reader who needs the values reads the file. The disclosure is reported at the top of the result on the MCP and HTTP surfaces alike.

- **A configuration document written back differs from the document it was read from only where a change was made to it (REQ-d00299-C)** -- a configuration file is written by hand and carries comments explaining why a setting is what it is, so a writer that reformats one on its way past is a writer nobody can afford to let near one. One property of TOML's model sits behind every way those bytes can be lost: a table's body runs from its header to the next header, so the comments and blank lines written between a declaration's last setting and the next header are stored inside the first table while belonging, to a reader, with what follows. Every edit lifts that text off before it works and puts it back where it was written. A change edits the declaration setting by setting rather than assigning its table whole: a setting the change leaves out is still a setting the project no longer states, but the note beside a setting nobody touched, the comment explaining why a setting is what it is, and the inline comment on the setting that did change all survive it. A removal takes the declaration and the blank line that separated it from what followed, because that blank belonged to the declaration being removed, and leaves the comment introducing the next declaration where it was written. An addition goes in front of the comment introducing whatever follows the declarations rather than after it. A rename is a respelling in place, and a document nobody changed is never written at all. The save path and the renderer carry none of this: a renderer dumps the document it is given, so the bytes are kept by the edit rather than recovered by the write.

- **A change that would leave a configuration the tool cannot load is refused (REQ-d00299-E)** -- the tool can no longer be disabled from its own interface. The refusal comes before the change rather than after it: the edit is applied to a copy of the held document, the copy is judged, and only a copy that will load is installed, so the document the graph is serving is never the one being judged. A change cannot produce an unparseable document, because `tomlkit` edits structure rather than text; it can produce an invalid one -- a document that parses and then fails schema validation -- and that is what is refused. The judgement goes through the same function that decides whether a configuration loads when it is read, so there is one answer to that question and the refusal at a change cannot drift from the refusal at a read; the message carries that underlying reason and names the document that was not changed. A refused change leaves nothing behind: the document is byte-identical, the derived configuration is what it was, no mutation joins the log and the count of pending changes does not rise. What is judged is shape, not meaning -- a key the schema does not define, a value of the wrong type and a name the schema will not admit are refused, while a scope selecting a level or a status no project declares is a configuration that loads and goes through.

- **A scope a project declares under a name can be added, changed, renamed and removed through the graph (REQ-d00299-D)** -- the four are ordinary graph mutations: they join the one mutation log, the one undo path and the count of pending changes with no special case, and they mark the configuration document dirty so the existing save path writes it. In a federation the mutation is recorded through the member that owns the document, so a save carries it rather than discarding it. A change states what the declaration says afterwards whole, so a setting the change leaves out is a setting the project no longer states. A declared name is matched without regard to case, so declaring a second name differing from an existing one only in case is refused naming the one already there, while renaming a declaration to another casing of its own name is a recasing and goes through. A rename respells the key where it stands rather than removing it and adding it back, so the declaration keeps its position. An undo restores the document's text rather than the values it held: a configuration is written by hand, and reversing values would reflow the comments, spacing and key order written around them. Which configuration tables are writable this way stays narrow and stays in one place -- a setting becomes changeable when someone has written the means to change it coherently -- and a table outside that set is refused. One consequence the tool reports rather than repairs: a comment written above a declaration belongs, in TOML's own model, to the item before it, so removing a declaration leaves such a comment in the document, where it now sits above whatever followed. A tool on the agent surface and a route on the viewer's reach each of them, each under a version token.

- **A scope a project declares under a name is addressable in the graph (REQ-d00299-D)** -- each `[scopes.NAME]` a configuration document declares is now held as a node of its own beneath that document, carrying the name it was declared under, the table it was declared in, and what it declares: the requirements a report produced under it is about and the values it states. Each carries a version of its own, taken from its own declaration, so a change to one scope does not invalidate a token held against another and two people editing two scopes do not collide. A declaration is addressed by the name it was declared under, without regard to case, and where both the committed document and the machine-local overlay declare a name, the one the derived configuration resolves to is the one found. These nodes are held beneath the document for addressing and versioning alone: a configuration document's text is the document, so rendering one still gives back the bytes it was read from and never walks what it holds.

- **The configuration a consumer reads is derived from the documents held in the graph (REQ-d00299-B)** -- the values and the document they came from had two independent readings of the same files and could come to disagree. The merging, the `[levels]` replacement, the refusals and the schema validation are now one function of the documents alone, and both readers call it: reading configuration from a path parses the documents and derives from them, and a consumer holding a repository's graph derives from the documents that graph holds. The dictionary is the one every command received before, key for key. The derivation stays a function of the documents and never of a graph, because reading configuration is what tells the builder where to look, and a derivation cached against a held document is taken again as soon as that document changes.

- **A repository's configuration is held in its graph (REQ-d00299-A)** -- building a repository's graph now holds a node for the configuration document it was read from, and for the machine-local overlay beside it where one exists, carrying the parsed document rather than the values taken from it. The document gives back the file it was read from exactly: comments, blank lines and the order its author chose. What changed is that the document is no longer discarded the moment its values have been read. A configuration document is a file type of its own, and a walk over files acts only on the types it names (REQ-d00131-R), so every existing report, scan and health check sees exactly what it saw before.

- **A pull request is opened from the viewer (REQ-d00297)** -- `POST /api/git/pr` proposes the branch the session has pushed, and a single-repository push offers it as a small dialog the moment it succeeds, with the title prefilled from the checkpoint suggestion. The pull request is attributed to whoever's credential opened it. A server started with a proxy secret is answering for whoever the proxy names, so the credential is the one the request carried and there is no fallback: a request without one is refused naming what to supply, rather than attributed to whatever identity the serving machine happens to hold. A server started without a secret is its operator's own viewer and reads `GH_TOKEN`, `GITHUB_TOKEN` or the credential helper as before. A branch the remote has not seen, a branch whose standing against the remote cannot be read, a detached HEAD and a remote that is not GitHub's are each refused naming the condition met, and where the remote already holds an open pull request proposing the same branch, that pull request is reported as the outcome.

  A credential a request carries now also serves the push and the fetch that request asks for. It is handed to that one git process on its own environment, which the operating system shows to the owning account alone, rather than on a command line, which it shows to every local account; it reaches neither the repository's configuration, the environment the server was started from, nor any file, and only an `https` remote is given one.

- **A report is exported from the viewer (REQ-d00298)** -- `GET /api/export/{report}?format=markdown|csv|pdf` downloads `trace`, `summary`, `gaps` or `checks` as a document, and the served page's toolbar gains an Export control that fetches it. The route takes exactly the query the matching `/api/run/{report}` route takes, read by the same builder, so what downloads is what was shown, narrowed by the level and status selection the page is showing under; a markdown or CSV download is the same formatter over the same payload as the command line's for the same inputs, so for those inputs it states the same values in the same layout, byte for byte, and a PDF is that markdown converted by pandoc through the template `elspais pdf` typesets with.

  An export is refused rather than delivered short: a format the report does not offer is a 400 naming the offered set, a missing converter is a 409 naming what to install, and a conversion that fails, or that could not fetch a resource the document names, is a 500 carrying its output. The attachment is named for its report and format. Only the level and status narrowings travel -- a scope can express no other, so a download taken while the page is narrowed by repository, coverage, git state or hierarchy is wider than the page, as the reports topic says.
...
- **A test reads as failing only where one of its own results failed (REQ-d00254-G, REQ-d00294-E)** -- a source-matched result that bound at neither step nor test scope used to be attached to every test in its file, so one failing record -- a sibling's, or a runner's own `(tearDownAll)`, `X (setUpAll)` or `loading <file>` pseudo-test -- made every test in the file read as failing on every surface but the coverage figures. Such a result now binds to no test: it credits nothing, it is still counted among the run's results, and `tests.unmatched_results` reports it saying why it bound nowhere (no line, a line where no scanned test starts, or a file where no test was scanned), with the environment it ran in beside it as `tests.results` shows one (REQ-d00294-F). A JUnit report carrying `file` but no `line` in a file holding several tests is the common case; the shipped Playwright reporter writes both. A record with no line in a file holding exactly one scanned test still binds to it where its name is that test's title -- the first argument of the test's declaring call, where that argument is a string literal (REQ-d00284-B) -- so a Playwright grid read from the built-in JUnit reporter keeps its results, and now carries their verdict, even where uncited tests of the same file ran beside it. A record of an unscanned sibling, including every record of a filtered run that ran only that sibling, binds to nothing, and so does a record ending in the scanned title where that title is not the scanned test's alone -- two differently grouped records both end in it, or the file declares it more than once with the same call (`test('x')` in two `describe`s, since the groups a test sits in are not read from its source) -- since which of them is the scanned test cannot be told. The parametrizations of one pytest function are that one test, and a title written as a raw or triple-quoted literal (Dart's `r'...'`, `'''...'''`) is read as its text. `flutter test`'s `loading <file>` record for a file that failed to load, and pytest's record of a module that failed to import or skipped itself at load (`pytest.importorskip`), is the one record standing for every test in its file, since none of them ran, and is each one's outcome.

- **`flutter test --machine` output is read as the runner judged it (REQ-d00254-E+Q, REQ-d00294-A+E, REQ-d00285-G)** -- an `error` event now reaches the result of the test it names: one reported after the test's `testDone` ("This test failed after it had already completed.") makes that result failing, and an error's text becomes the result's message. A test the stream started and never finished is recorded as failing, and a stream that ends without its `done` event is reported under `tests.partial_read` (or `tests.ingestion_fault` where nothing finished), so a crashed or killed run no longer reads as a test nobody selected. A run the runner reported as `success: false` with no failing result is reported rather than read as all-passing. A record read from a saved `.jsonl` now carries the line of its `testDone`, so a failing result points at its record.

- **A relative path a runner records is read against the directory it ran in (REQ-d00254-F+G, REQ-d00294-E)** -- `dart test` writes `suite.path` relative to its package and pytest writes a JUnit `file`, a classname and a nodeid relative to its rootdir, so a target with a `cwd` bound none of its results, or bound them to a same-named file elsewhere. Such a path is now read against the target's `cwd` wherever that names a scanned test, and a producer already writing repo-relative paths keeps what it wrote; a target run from the repository root reads as before.

- **A result is placed by the artifact that holds it whichever channel was read (REQ-d00294-A+B)** -- a target declaring `results` is now read from that artifact even where a run also captured its output, provided every file the pattern matches was written during that run, so one run's result keeps one identity and the artifact its FILE node. An artifact older than the run -- an earlier run's report, left in place by a run that failed before its file reporter opened -- is not read in place of the output this run captured, so a stale pass never hides a fresh failure. The captured output is read for a target declaring no artifact, or whose artifact the run did not leave, and for each captured target whose artifact another captured target also declares: whichever ran last wrote that file, and nothing on it says which, so reading it would hand every such target the last one's verdicts and discard the rest. A target that did not run -- every target, in a build without `--run-tests` -- does not read an artifact another target also declares, since it may hold a result another target has just written under the same identity; `tests.ingestion_fault` reports it instead.

- **A citation above a test an external prescan reports binds to that test (REQ-d00254-D+N)** -- a prescan record's range ran to the next record's line whatever `end_line` it declared, so the comment block written above every test after the first lay inside the test before it and each citation was attributed one test early. A declared `end_line` now bounds the record, and the lines after it are bound forward to the test they were written above.

- **Each `flutter test --machine` run in one stream reads its own identifiers (REQ-d00294-A+E)** -- a command running the runner more than once writes runs back to back, each numbering its tests from 1; an error reported by the second run no longer lands on the first run's test of the same number, and the first run's `done` no longer hides a second run cut short.

- **Every JUnit record is held and points at its own line (REQ-d00294-A)** -- the `<testcase>` elements of a root `<testsuite>` that also holds nested suites were dropped; every suite in a document is now read. Two records with one classname and name -- one per Playwright project, or pytest's second record for a teardown error -- now each point at their own `<testcase>` line rather than all at the first.

- **A pytest JSON outcome the reporter cannot read is reported, not read as a pass (REQ-d00285-G, REQ-d00294-E)** -- a missing or unrecognised outcome was credited as passing. It is now compared without regard to case or surrounding space (`failure` reads as failed), a `rerun` attempt is left to the test's own record, and any other record is declined and reported under `tests.partial_read`.

- **The environment read from a results path is the part its wildcard matched, or a reported fault (REQ-d00294-C+D)** -- a wildcard matching nothing (`junit*.xml` against `junit.xml`) gave an empty environment silently and now reports that none was derived; a bracket class is read the way the glob that selected the file reads it, so `[!x]` negates rather than producing a false fault; and case folds exactly where the glob folded it.

- **A results artifact reached through a symlink keeps its repo-relative place (REQ-d00294-A+B)** -- a results directory linked to one outside the repository gave its FILE node and its results absolute ids; the path the file is written under inside the repository is now used first.

- **Two test targets may not share a name (REQ-d00294-A+B)** -- a target's name selects it for a run, keys its captured output and places the results read from that output, so two targets named alike overwrote one another and a failing record vanished. The configuration is now refused naming both, compared without regard to case or surrounding space.

- **The shipped Playwright reporter writes outcomes, parses, and lands where its configuration says (REQ-d00294-A+E)** -- it wrote one record per attempt, so a flaky test that passed on retry and a `test.fail()` test that failed as expected read as failing; it now writes one record per test per project with Playwright's own outcome. ANSI sequences and characters XML forbids are stripped, so a failing run's report parses. `outputFile` and each `file` are resolved against the Playwright config's directory rather than the test directory, and the header's example target carries `line_base = 1`.

- **The e2e verdict hook records a verdict only for a run that tested the tree** -- `.githooks/e2e-verdict` cached FAIL for a run in which pytest was never found (exit 127), could not collect or was stopped, and re-formatted the previous run's report as though it were this one's. It now removes the previous report before a run, hands the run the resolved pytest, and records PASS only for exit 0 with a report holding no failure and FAIL only for a failing exit (1, or 2 for a collection error) with a report holding one -- an interrupt whose report holds no failure records nothing; anything else leaves the cache untouched and exits non-zero naming the cause.

- **A save discarded the changes it declined to write (REQ-d00132-E, REQ-d00253-B, REQ-d00253-D, REQ-d00253-G, REQ-o00062-O)** -- a save writes only the repository the tool is serving unless `federation.write_associates` says otherwise, and it clears the pending changes once it has succeeded. A change queued against a file an associate owns met both of those at once: the file was held back, the pending record was cleared anyway -- and clearing it in a federation reaches into every member's own record -- so the save reported success, wrote the serving repository's file, left the associate's untouched, and destroyed the edit made to it with nothing in the report saying so.

  A save that cannot write every file the work requires now writes none of it (REQ-d00253-G). A change is not always confined to the file its author edited: renaming a requirement corrects the reference in every file citing it, and those files may belong to other members, so writing the part the write scope reaches would leave a repository citing an identifier that no longer exists -- a reference broken by the save itself, in a file nobody edited, in a repository the operator may not be looking at. Holding all of it back leaves every member as it was and the work still in hand: no file is rendered, not even one the serving repository owns, the pending record is untouched, and the recourse is to widen the write scope and save again or to save from a tool serving the repository that owns the file. A held-back file that carries queued work is named to the caller, in what the save reports as skipped and as an error, so nothing is withheld in silence; a file that is merely parse-dirty -- formatting the tool would tidy, which nobody asked for -- is still held back without a word and triggers none of this, so `elspais fix` in a federation is unaffected.

  An unsuccessful save also says which kind of unsuccessful it was, because a caller can act on one and not the other: a save that held anything back carries `write_scope_declined` and an error naming those files in full, and the viewer answers it with 403 rather than the 500 a failed write gets, so a policy answer a further save will repeat is told apart from a write that may succeed next time. A decline never reaches a write, so it is never also a failure. The MCP save tool and the viewer's route read the one save result, so both report the same code and the route hands the save's own body back rather than re-wording it; the viewer's page passes a refusal that names itself to the caller that raised it instead of collapsing it into a toast.

  The viewer's mutation routes now refuse a target an associate owns before they judge the version token, in the order the MCP tools check in and returning the tools' own refusal verbatim under HTTP 403: a write the agent surface refuses and the viewer accepts is a mutation reachable on one surface alone, and accepting it for an associate-owned node only queued an edit the save would then decline to write.

  Write and index eligibility is one setting for every member, and declaring it per associate entry is retired (REQ-d00253-E): which member a write may reach is decided by where the work happens rather than by a declaration. A workspace prepared for one person holds that person's own clone of every member on one branch, so a member it prepared is writable; where nobody prepared the checkouts the hazard is the same across every member, and the single setting states it once.

- **The configuration layers were documented as four and are three** -- `get_config`'s docstring and the `config` documentation topic both gave the precedence as ending in environment variables, and the topic said in one place that the environment always wins and in another that it supplies no configuration at all. Nothing reads the environment for a setting: the layers are the schema's defaults, `.elspais.toml`, and `.elspais.local.toml` over it. Both now say so, and the config module's export list no longer names a value-parsing helper that went with the environment layer and no longer exists, which made `from elspais.config import *` fail.
...
- **A file's content comes from the renderer its file type declares (REQ-d00131-I, REQ-d00131-Q)** — rendering a FILE node is a dispatch on the file's type rather than one hard-coded strategy. The spec, journey, code and test types declare the walk over the nodes the file holds, in render order, which is what every one of them rendered before and renders now, byte for byte. A results artifact declares no renderer, so asking for its content is refused naming the type: results are read-only by statement now rather than by the accident of nothing composing them. A type that declares neither is refused too, so a type added later is read-only until it says how it is written.

- **The viewer's pull is fast-forward only (REQ-p00004-F, REQ-d00297-F)** -- `POST /api/git/pull` fetches and fast-forwards from the branch's own remote-tracking ref. It no longer merges a diverged history or replays the branch onto main: reconciling two histories needs decisions about content elspais does not make, so work that cannot be fast-forwarded is handed back with the branch left exactly as it was found, and the user is told to push the branch and finish it in a local checkout.
