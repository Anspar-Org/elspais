...
- **Precise (file-granular) matching now wires real RESULT→TEST edges, so the viewer's per-assertion Verified panel populates for `flutter-machine` results** (CUR-1533) — `match = "precise"` credited the `verified` *metric* via a source-file path-index in the annotator, but never created the RESULT→TEST `YIELDS` graph edges that "file-granular RESULT→TEST verification by real path" (REQ-d00254-G) implies. The viewer's per-assertion test map (`_get_assertion_test_map` → `_serialize_test_info`) reads a TEST node's RESULT *children*, so for test_id-less reporters (`flutter-machine` emits no `test_id`) every assertion's VER panel was empty even when the requirement showed verified. The builder now resolves each precise, test_id-less result to the TEST nodes sharing its real `source_file` (no match → no edge, never a broken reference). Crediting is unchanged: these file-granular edges are skipped by the annotator's per-test loop and credited through the existing file-level `precise_index` (all-pass credits / any-fail flags the whole file), so the `verified` metric — including the conservative mixed-pass/fail semantics — is byte-identical; only the viewer gains the per-test results it was missing. test_id-linked results (junit/pytest, and any precise result that carries a test_id) continue to credit per-test as before.
