...
**Iterator-Only Graph API**: The graph uses an iterator-only API to prevent accidental list materialization:
  ...
  - **TraceGraph**: Use `iter_roots()` for traversal (accepts optional `NodeKind` filter: `iter_roots(NodeKind.FILE)` for FILE nodes, `iter_roots(NodeKind.REQUIREMENT)` for REQ roots only; default returns REQ+JOURNEY roots excluding FILE). Use `iter_by_kind(kind)` for general kind-based index queries. Use `root_count()`, `has_root()` for checks. Internal storage uses `_roots`, `_index` prefixed attributes.
