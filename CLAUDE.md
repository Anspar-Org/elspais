...
24. **MCP Test Coverage Tools** (`elspais[mcp]`): The MCP server provides tools for analyzing test-requirement relationships:
    - **`get_test_coverage(req_id)`**: Returns TEST nodes referencing a requirement, their TEST_RESULT nodes, and coverage statistics (total_assertions, covered_count, coverage_pct).
