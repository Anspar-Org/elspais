...
**Satisfies Relationship**: `Satisfies:` metadata declares compliance with a template requirement (e.g., a cross-cutting PRD). Coverage is computed per-instance via the `satisfies_coverage` metric, tracking the fraction of the template's leaf assertions covered within the declaring requirement's subtree. Assertions declared as `REQ-xxx-Y SHALL be NOT APPLICABLE` exclude template assertions from the coverage denominator. `check_template_coverage()` health check reports gaps. Template hash changes flag declaring requirements for review.
