...
**Unified References Configuration** (`[references]`): Configurable reference parsing for all parser types with `enabled` and `case_sensitive` fields. Separators and prefix_optional are now in `[id-patterns]`.
