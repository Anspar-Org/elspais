...
``_relative_tier`` measures one ``CoverageDimension`` against a RELATIVE
...
from elspais.html.generator import _relative_tier
...
def test_empty_denominator_is_na():
    ...
    dim = _tested({"A"})
    assert _relative_tier(dim, set()) == ("missing", True)
...
def test_allow_indirect_true_credits_blanket_coverage():
    ...
    assert _relative_tier(dim, {"A", "B"}, allow_indirect=True) == ("full", False)
...
def test_allow_indirect_false_excludes_blanket_coverage():
    ...
    assert _relative_tier(dim, {"A", "B"}, allow_indirect=False) == ("missing", False)
...
def test_allow_indirect_false_credits_direct_labels():
    ...
    assert _relative_tier(dim, {"A", "B"}, allow_indirect=False) == ("partial", False)
