...
def _tested(labels, *, direct=None, failing=()):
    ...
    tests can exercise the ``measure="direct"`` path independently.
    """
    labels = set(labels)
...
def test_nonempty_denominator_zero_covered_is_real_gap():
    ...
    assert relative_tier(dim, {"A", "B"}, measure="indirect") == ("missing", False)
...
def test_allow_indirect_true_credits_blanket_coverage():
    """Indirect-only coverage counts when measure="indirect" (the default)."""
...
def test_allow_indirect_false_excludes_blanket_coverage():
    """Indirect-only coverage is NOT credited when measure="direct"."""
    dim = _tested({"A", "B"}, direct=set())  # direct_pct empty
    # With no direct credit and non-empty denom, this is a real gap.
    assert relative_tier(dim, {"A", "B"}, measure="direct") == ("missing", False)


def test_allow_indirect_false_credits_direct_labels():
    """Direct coverage is credited under measure="direct"."""
    dim = _tested({"A", "B"}, direct={"A"})  # only A has direct credit
    assert relative_tier(dim, {"A", "B"}, measure="direct") == ("partial", False)
