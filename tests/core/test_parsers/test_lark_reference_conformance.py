...
@pytest.fixture
def code_parser(resolver, ref_config):
    factory = GrammarFactory(resolver, ref_config)
    return factory.get_reference_parser()


def _parse_code(content, resolver, ref_config, code_parser):
    """Parse as code_ref."""
    if not content.endswith("\n"):
        content += "\n"
    tree = code_parser.parse(content)
    tx = ReferenceTransformer(resolver, "code_ref", ref_config)
    return tx.transform(tree)


def _parse_test(content, resolver, ref_config, code_parser, **kwargs):
    """Parse as test_ref."""
    if not content.endswith("\n"):
        content += "\n"
    tree = code_parser.parse(content)
    tx = ReferenceTransformer(resolver, "test_ref", ref_config, **kwargs)
    return tx.transform(tree)
...
class TestCodeRefParsing:
    ...
    def test_single_implements(self, resolver, ref_config, code_parser):
        content = "# Implements: REQ-p00001\ndef foo(): pass\n"
        results = _parse_code(content, resolver, ref_config, code_parser)
    ...
    def test_single_verifies(self, resolver, ref_config, code_parser):
    ...
    def test_multiple_refs_comma_separated(self, resolver, ref_config, code_parser):
    ...
    def test_block_header_and_refs(self, resolver, ref_config, code_parser):
        content = """\
...
"""
    ...
    def test_js_style_comments(self, resolver, ref_config, code_parser):
        content = "// Implements: REQ-p00001\nfunction foo() {}\n"
        results = _parse_code(content, resolver, ref_config, code_parser)
        refs = [r for r in results if r.content_type == "code_ref"]
        assert len(refs) == 1
    ...
    def test_no_refs_in_plain_code(self, resolver, ref_config, code_parser):
        content = "def foo():\n    return 42\n"
        results = _parse_code(content, resolver, ref_config, code_parser)
        refs = [r for r in results if r.content_type == "code_ref"]
        assert len(refs) == 0
...
class TestTestRefParsing:
    ...
    def test_verifies_comment(self, resolver, ref_config, code_parser):
        content = "# Verifies: REQ-p00001\ndef test_something(): pass\n"
        results = _parse_test(content, resolver, ref_config, code_parser)
        refs = [r for r in results if r.content_type == "test_ref"]
        assert len(refs) >= 1
        assert refs[0].parsed_data["verifies"] == ["REQ-p00001"]

    def test_test_name_pattern(self, resolver, ref_config, code_parser):
    ...
    def test_file_default_verifies(self, resolver, ref_config, code_parser):
    ...
    def test_control_marker_recognized(self, resolver, ref_config, code_parser):
    ...
    def test_block_verifies(self, resolver, ref_config, code_parser):
        content = """\
