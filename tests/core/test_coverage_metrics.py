...
class TestRollupMetrics:
    ...
    def test_finalize_computes_aggregates(self):
        ...
        metrics.add_contribution(
            CoverageContribution("test:1", CoverageSource.DIRECT, "A")
        )
        metrics.add_contribution(
            CoverageContribution("req:2", CoverageSource.EXPLICIT, "B")
        )
    ...
    def test_multiple_contributors_same_assertion(self):
        ...
        metrics.add_contribution(
            CoverageContribution("test:2", CoverageSource.DIRECT, "A")
        )
...
class TestUserExample:
    ...
    def test_user_example_scenario(self):
        ...
        assert any(
            c.source_type == CoverageSource.DIRECT
            for c in metrics.assertion_coverage["A"]
        )
