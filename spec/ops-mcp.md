...
I. A daemon that stops while holding unsaved changes SHALL persist them rather than discarding them, whatever prompted it to stop, SHALL record that the daemon itself performed the save rather than a client requesting it, when it did so, how many changes it covered, and the condition that triggered it, and SHALL disclose that record to clients that afterwards work with the affected files.
...
*End* *Background Daemon Lifetime* | **Hash**: f47c8b15
---
