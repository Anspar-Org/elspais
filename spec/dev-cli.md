...
C states the shape of a scope and D, E and F say what it selects. A scope is a set of properties, each carrying values, and C is what keeps any combination of them writable — a reader who wants two levels and three statuses should not have to discover which pairings the tool happens to support. D, E and F then fix the reading, because a combination whose meaning is left open is not a vocabulary. Properties named together are conditions a requirement meets at once; values named for one property are alternatives. That is the same narrowing a reader performs when browsing the estate interactively, and a vocabulary that meant something else would give a reader two incompatible ways to say one thing. Stating the two directions apart is what keeps a scope's meaning independent of the order its terms were written in.

F is stated as a refusal rather than as a second kind of scope, which is what lets exclusion and requirement meet on one property without the vocabulary needing a rule for their collision. An audience defined by what it does not want is defined against a set that grows, and a vocabulary offering only the positive form obliges such an audience to be rewritten each time the project adds a status. Because F denies satisfaction outright, a property carrying both forms resolves the same way wherever they overlap, and an author reasoning about either form is reasoning about one selection.

G is what makes REQ-p00084-G reachable for statuses, and it is deliberately not a property of its own. A project's statuses are its own and grow; the roles it assigns them are the stable vocabulary underneath. Were a role a value the status property admitted, a reader would face two things that look alike in the same list and behave differently, and every scope would have to be read to discover which it named. Keeping the role out of the value list and making it a widening of the values already named leaves one kind of thing in the list and one question about the scope as a whole: whether a named status stands for itself or for everything sharing its role. An author who wants the narrow reading writes what they mean and gets it.

H and I divide a question that would otherwise be answered by whichever member happened to be asked. H says what a vocabulary holds. A value a requirement carries is part of the vocabulary whether or not the configuration names it, because a requirement carrying a status the project never listed is a requirement a reader can see and therefore one a reader can ask for; leaving it unnameable would make a scope unable to describe an estate the tool itself displays. I says whose vocabulary is consulted. Each requirement is judged under the vocabulary of the project that wrote it, never under one assembled from several; a merged vocabulary would let one member's configuration decide what another member's requirements are, and would make a scope's meaning depend on which repository happened to ask for the report. REQ-d00251-L settles the same question for identifiers, and the reasons carry over unchanged. The two properties are not governed alike — the roles a project assigns to statuses are settled across a federation, while the levels a project defines are its own — so the vocabulary a scope keyed on level speaks varies member by member in a way the role vocabulary does not, and H and I have to hold for the harder of the two.

J, K and L are the honesty group, and they are separate because opposite situations produce the same thin report. A name the vocabulary does not admit selects nothing while looking like it selected something, and a federated estate is where that hides best: members configure their own levels and their own statuses, so a name one member admits and another does not is an ordinary state of affairs that quietly removes that member's requirements from the answer. J is the disclosure; K is what keeps the disclosure from becoming a refusal, since a cross-member scope naming a level only some members define is a legitimate scope and a report that stops at the first such name is useless to the reader who wrote it. L is the other side: a scope selecting nothing is frequently the correct result — an audience whose requirements have all been withdrawn, a status nothing carries yet — and reporting it as the answer keeps it distinguishable from the conditions REQ-d00080-B and REQ-d00080-E report, which are about an estate or a member holding no requirements at all rather than about a question that legitimately has no members in its answer.
...
- 2026-08-20 | - | - | Michael Lewis (<michael@anspar.org>) | Initial authoring: level and status as selectable properties, any combination of properties and values admitted, conditions across properties and alternatives within one, exclusion stated as a refusal, a status widened to its role on request rather than roles standing as status values, a property's vocabulary defined and resolved only in the owning member, unadmitted names disclosed without refusing the report, empty selections reported as answers, and room for further properties
...
C settles the name across a federation. A scope name is a label for a whole selection, not a value inside one, so the question REQ-d00278-I answers for the properties and values a scope names does not answer this one: the members a report spans each hold their own configuration, and a name declared in several of them would otherwise stand for several selections in a single report. Reading the name in the repository the report is asked of gives it one meaning and one owner, and leaves the selection it stands for to be resolved member by member as REQ-d00278-I requires — so a name declared once still means each member's own levels and statuses when it is applied to that member's requirements.

D and E are the pair a misspelling needs. An undeclared name has no selection behind it, so a report produced under one narrows to nothing or to everything depending on how the absence is read, and either way it looks like an answer. D makes the absence visible; E is what keeps it from being papered over, and it is the opposite disposition from the one REQ-d00278-K takes for a value the vocabulary does not admit. That difference is deliberate. An unadmitted value sits inside a scope whose remaining terms still say something the author meant, so the report can go on with the disclosure attached. An unadmitted name is the whole of what the author said, so continuing means the tool choosing a selection nobody asked for.
